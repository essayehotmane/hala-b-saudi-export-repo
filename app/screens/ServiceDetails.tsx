...
const ServiceDetails = ({ route }) => {
  ...
  useEffect(() => {
    const getTokenAndFetchFavorites = async () => {
      try {
        const _token = await dispatch(issueToken(connectedUser.phoneNumber?.replace("+", ''))).unwrap();
        const savedUser = await dispatch(getUserFromStorage()).unwrap();
        setUserId(savedUser.id);
        setToken(_token.token);
        await dispatch(fetchFavorites({userId: userId, token: _token.token})).unwrap();
      } catch (error) {
  ...
  const getDiscountCode = async (serviceId: number, userId: number, token: string) => {
    try {
  ...
  const handleGetNewCode = (serviceId: number, userId: number, token: string) => {
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: () => dispatch(generateDiscountCode({ userId, serviceId, token })),
        },
      ],
      { cancelable: false }
    );
  ...
  return (
      <View style={styles.container}>
        ...
        <DiscountModal visible={showModal} onClose={()=> setShowModal(false)} getNewCode={()=>{handleGetNewCode(service.id, userId, token)}} code={code} title={capitalizeFirstLetter(t('your_discount_code_is'))} paragraph={capitalizeFirstLetter(t('single_use_1')) + ' ' + capitalizeFirstLetter(t('single_use_2'))} buttonText={capitalizeFirstLetter(t('get_a_new_code'))}/>

        <HeaderLogo />
        <View style={styles.imageContainer}>
          <ServiceImage uri={service.logo} serviceId={service.id} userId={userId} />
        </View>
        ...
        <View style={styles.serviceDetail}>
          ...
          <Text numberOfLines={4} ellipsizeMode="tail" style={styles.desc}>{capitalizeFirstLetter(service.desc)}</Text>
        </View>
        ...
        <View style={styles.buttonContainer}>
          ...
          >
            <Text style={styles.getDiscountText}>{CapitalizeFirstLetter(t('get_a_discount_code'))}</Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <Text style={styles.openMapsText}>{CapitalizeFirstLetter(t('open_in_maps'))}</Text>
          </TouchableOpacity>
        </View>
      </View>
  );
