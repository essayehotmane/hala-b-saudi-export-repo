  ...
  async (
    {
      ...
      userId,
      token,
    }: {serviceId: number; userId: number; token: string},
    {rejectWithValue},
  ) => {
...
>('discount/fetchDiscounts', async ({userId, token}, {rejectWithValue}) => {
  console.log('getting discounts...', userId, token);
  try {
    const response = await axios.get(`${consts.API_URL}/discounts`, {
...
const discountSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        generateDiscountCode.fulfilled,
